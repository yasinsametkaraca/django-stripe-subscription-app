...
def home(request):
    ...
    if request.user.is_authenticated:
        try:
            ...
            print(profile.is_pro)
        except Profile.DoesNotExist:
            request.session['profile'] = False
...
def become_pro(request):
    if request.method == 'POST':
        ...
        customer = stripe.Customer.create(
            email=request.user.username + "@gmail.com",  # add email
...
def register(request):
    ...
    if request.method == "POST":
        ...
        if user:
            ...
            user.save()
            Profile.objects.create(user=user, is_pro=False, subscription_type='F')
